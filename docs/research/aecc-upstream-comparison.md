...
| Resolved 2026-09-26 | Restore self-consumption with both native AI flags enabled | The FBP1200 accepted and read back `3020=3, 3021=0, 3022=1, 3030=0`, while Home Assistant still measured 0 W battery output under a 158 W connected load at 90% SOC. The current AECC upstream restores self-consumption with **both** AI flags enabled (`3021=1`, `3022=1`), custom off, and a cleared slot ([register map and mode sequence](https://github.com/StekkerDeal/aecc-battery-local/blob/099e3bb078eea8ddfa9cf145b57e7e8da6cf73a8/custom_components/aecc_battery/const.py#L182-L265)). | House Battery now writes both AI flags and verifies the complete absolute register set. Transport retries remain bounded and read-back-driven; an acknowledgement alone is not physical proof of movement. |
...
3. Retain a redacted hardware fixture for the confirmed self-consumption flags
   and collect an app-write capture if a future firmware changes the `3003`
   slot variant.
